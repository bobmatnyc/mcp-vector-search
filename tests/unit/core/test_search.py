...
from mcp_vector_search.core.models import SearchResult, CodeChunk
...
class TestSemanticSearchEngine:
    ...
    async def test_search_basic_functionality(self, mock_database, sample_code_chunks):
        ...
        # Execute
        results = await search_engine.search("main function", limit=5, similarity_threshold=0.1)
    ...
    @pytest.mark.asyncio
    async def test_search_auto_indexer_error_handling(self, mock_database, sample_code_chunks):
    ...
    async def test_result_enhancement(self, mock_database, sample_code_chunks):
        ...
        async with mock_database as db:
            ...
            # Test enhancement
            enhanced_result = await search_engine._enhance_result(basic_result, include_context=True)
    ...
    @pytest.mark.asyncio
    async def test_performance_characteristics(self, mock_database, sample_code_chunks, performance_timer):
        ...
        async with mock_database as db:
            ...
            assert stats["average"] < 0.5, f"Average search time too high: {stats['average']:.3f}s"
