...
class TestSemanticIndexer:
    ...
    @pytest.mark.asyncio
    async def test_performance_large_project(self, mock_database, temp_dir, performance_timer):
        ...
        assert elapsed < 30.0, f"Indexing took too long: {elapsed:.3f}s"  # Should complete in reasonable time
        
        # Verify indexing worked
        stats = await indexer.get_indexing_stats()
        assert stats["indexed_files"] == len(test_files)
        assert stats["total_chunks"] > len(test_files)  # Should have multiple chunks per file
