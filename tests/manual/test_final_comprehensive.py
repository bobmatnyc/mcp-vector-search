...
async def comprehensive_test():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            results["tests"]["server_connectivity"] = {"status": "FAIL", "error": str(e)}
            print(f"❌ Error: {e}")
            return results
        ...
        structure_pass = (
            page_info['svgCount'] == 1 and
            page_info['nodeCount'] > 0 and
            page_info['hasD3'] and
            page_info['controlsVisible'] and
            page_info['graphVisible']
        )
        ...
        print(f"{'✅' if structure_pass else '❌'} SVG Elements: {page_info['svgCount']}")
        print(f"{'✅' if page_info['nodeCount'] > 0 else '❌'} Node Elements: {page_info['nodeCount']}")
        print(f"{'✅' if page_info['linkCount'] >= 0 else '❌'} Link Elements: {page_info['linkCount']}")
        print(f"{'✅' if page_info['hasD3'] else '❌'} D3.js Loaded: {page_info['hasD3']}")
        ...
        print(f"{'✅' if metrics['domContentLoaded'] < 3000 else '⚠️'} DOM Content Loaded: {metrics['domContentLoaded']}ms")
        print(f"{'✅' if metrics['loadComplete'] < 5000 else '⚠️'} Load Complete: {metrics['loadComplete']}ms")
        print(f"{'✅' if metrics['domInteractive'] < 2000 else '⚠️'} DOM Interactive: {metrics['domInteractive']}ms")
        ...
        # Take screenshot
        await page.screenshot(path='/tmp/visualizer_final_screenshot.png', full_page=True)
    ...
    print(f"📸 Screenshot: /tmp/visualizer_final_screenshot.png")
