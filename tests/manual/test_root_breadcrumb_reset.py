...
async def test_root_breadcrumb_reset(url: str = "http://localhost:8080"):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print(f"    Fragments detected: {after_reset.get('fragments_count', 'ERROR')}")
            ...
            if after_reset.get('expansion_path'):
                print(f"  ❌ FAIL: Expansion path not cleared")
                print(f"      Expected: []")
                print(f"      Got: {after_reset['expansion_path']}")
                errors.append("Expansion path not cleared")
                passed = False
            else:
                print(f"  ✅ PASS: Expansion path cleared")

            # Check 2: View mode should be tree_root
            if after_reset.get('view_mode') != 'tree_root':
                print(f"  ❌ FAIL: View mode not reset to tree_root")
                print(f"      Expected: tree_root")
                print(f"      Got: {after_reset['view_mode']}")
                errors.append("View mode not reset")
                passed = False
            else:
                print(f"  ✅ PASS: View mode reset to tree_root")

            # Check 3: No fragments (non-root nodes) should be visible
            if after_reset.get('fragments_count', 0) > 0:
                print(f"  ❌ FAIL: Fragments detected ({after_reset['fragments_count']} nodes)")
                print(f"      These are child nodes that should be hidden in Phase 1")
                errors.append(f"{after_reset['fragments_count']} fragment nodes visible")
                passed = False
            else:
                print(f"  ✅ PASS: No fragments detected (clean Phase 1 view)")
            ...
            if abs(reset_count - initial_count) > initial_count * 0.2:
                print(f"  ⚠️  WARNING: Node count differs significantly")
                print(f"      Initial: {initial_count}")
                print(f"      After reset: {reset_count}")
                errors.append(f"Node count mismatch: {initial_count} vs {reset_count}")
            else:
                print(f"  ✅ PASS: Node count consistent ({initial_count} → {reset_count})")
            ...
            print(f"\n{'='*70}")
            if passed:
                print("🎉 FINAL VERDICT: ✅ PASS - Root breadcrumb reset works correctly")
                print("   Clicking 'Root' now shows a clean Phase 1 grid with no fragments!")
            else:
