...
def test_root_node_detection():
    ...
    print(f"✓ Has directory root nodes: {has_directories} (found {len(by_type.get('directory', []))})")
    print(f"✓ Has file root nodes: {has_files} (found {len(by_type.get('file', []))})")
    print(f"✓ No function root nodes: {no_functions} (found {len(by_type.get('function', []))})")
    print(f"✓ No class root nodes: {no_classes} (found {len(by_type.get('class', []))})")
    ...
    if all_pass:
        ...
        print(f"  - 0 functions (they have parent files)")
        print(f"  - 0 classes (they have parent files)")
        return 0
    else:
