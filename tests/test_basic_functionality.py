...
async def test_indexing_and_search(temp_project_dir):
    ...
    async with database:
        ...
        results = await search_engine.search("function", limit=10, similarity_threshold=0.1)
        assert len(results) > 0, f"No results found. Total chunks: {stats['total_chunks']}"

        # Test search for any content
        all_results = await search_engine.search("def", limit=10, similarity_threshold=0.1)
