...
class SearchPerformanceTester:
    ...
    def __init__(self, database: ChromaVectorDatabase, search_engine: SemanticSearchEngine):
...
async def test_basic_search_timing(performance_tester: SearchPerformanceTester):
    ...
    assert stats['mean'] < 0.5, f"Average search time too slow: {stats['mean']*1000:.2f}ms"
    assert stats['max'] < 1.0, f"Max search time too slow: {stats['max']*1000:.2f}ms"


@pytest.mark.asyncio
async def test_search_with_different_limits(performance_tester: SearchPerformanceTester):
    ...
    assert ratio < 3.0, f"Performance degrades too much with higher limits: {ratio:.2f}x"


@pytest.mark.asyncio
async def test_search_with_different_thresholds(performance_tester: SearchPerformanceTester):
    ...
    for threshold in thresholds:
        ...
        print(f"  Threshold {threshold:.1f}: {elapsed*1000:.2f}ms ({len(result)} results)")
...
@pytest.mark.asyncio
async def test_concurrent_search_performance(performance_tester: SearchPerformanceTester):
...
@pytest.mark.asyncio
async def test_search_query_complexity_performance(performance_tester: SearchPerformanceTester):
    ...
    queries = [
        ...
        ("complex_technical", "implement sqlite database connection with error handling"),
    ]
    ...
    for query_type, query in queries:
        ...
        print(f"  {query_type:15s}: {elapsed*1000:.2f}ms ({len(result)} results) - '{query}'")
