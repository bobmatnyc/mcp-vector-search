...
class TestIndexingWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_full_indexing_workflow(self, temp_project_dir, real_embedding_function):
    ...
    @pytest.mark.asyncio
    async def test_incremental_indexing_workflow(self, temp_project_dir, real_embedding_function):
        ...
        async with DatabaseContext(components.database):
            ...
            incremental_count = await components.indexer.index_project(force_reindex=False)
    ...
    @pytest.mark.asyncio
    async def test_force_reindexing_workflow(self, temp_project_dir, real_embedding_function):
        ...
        async with DatabaseContext(components.database):
            ...
            assert force_count == initial_count, "Force reindex should process all files"
    ...
    @pytest.mark.asyncio
    async def test_single_file_indexing_workflow(self, temp_project_dir, real_embedding_function):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_workflow(self, temp_project_dir, real_embedding_function):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_indexing_workflow(self, temp_project_dir, real_embedding_function):
    ...
    @pytest.mark.asyncio
    async def test_search_integration_workflow(self, temp_project_dir, real_embedding_function):
    ...
    @pytest.mark.asyncio
    async def test_metadata_consistency_workflow(self, temp_project_dir, real_embedding_function):
    ...
    @pytest.mark.asyncio
    async def test_performance_workflow(self, temp_project_dir, real_embedding_function, performance_timer):
        ...
        async with DatabaseContext(components.database):
            ...
            for query in search_queries:
                ...
                assert search_time < 1.0, f"Search for '{query}' took too long: {search_time:.3f}s"
                assert isinstance(result, list)
            
            # Check overall performance stats
            stats = performance_timer.get_stats()
            assert stats["average"] < 5.0, f"Average operation time too high: {stats['average']:.3f}s"
