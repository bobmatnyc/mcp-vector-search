...
'''
        ...
        async with database:
            ...
            results = await search_engine.search("hello world", limit=5, similarity_threshold=0.05)
...
'''
        ...
        async with database:
            ...
            results = await search_engine.search("goodbye world", limit=5, similarity_threshold=0.05)
            ...
            results = await search_engine.search("multiply", limit=5, similarity_threshold=0.05)
        ...
        single_reindex_content = modified_content + '''
...
'''
        ...
        async with database:
            ...
            results = await search_engine.search("calculate area", limit=5, similarity_threshold=0.05)
        ...
        if metadata_file.exists():
            ...
            for file_path in actual_files:
                ...
                if rel_path in final_metadata:
                    ...
                    print(f"    {file_path.name}: stored={stored_mtime:.3f}, actual={actual_mtime:.3f}")
                else:
...
''')
        ...
        async with database:
            ...
            print(f"  Incremental indexing: {incremental_count} files in {incremental_time:.2f}ms")
            ...
            print(f"  Performance ratio: {force_time / max(incremental_time, 1):.1f}x slower for force reindex")
...
''')
        ...
        async with database:
            ...
            # Search for new content
            results = await search_engine.search("new function", limit=5, similarity_threshold=0.05)
