...
async def test_search_performance(database, search_engine, test_name: str, num_searches: int = 20):
    ...
    async with database:
        for i in range(num_searches):
            ...
            if i < 5:  # Show first 5 searches
                print(f"  Search {i+1} ('{query}'): {search_time:.2f}ms ({len(result)} results)")
...
async def test_concurrent_performance(database, search_engine, test_name: str, concurrency: int = 5):
    ...
    print(f"  Total results: {total_results}")
    print(f"  Concurrent throughput: {concurrency / (wall_time / 1000):.1f} searches/sec")
...
async def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"Sequential Search Performance:")
        ...
        concurrent_improvement = ((pooled_concurrent_throughput - regular_concurrent_throughput) / regular_concurrent_throughput) * 100
