...
each claim. Applies deterministic guards first, then calls Claude Opus as judge.
"""
...
async def judge_claim(
    ...
    4. LLM judge (Claude Opus) → PASS/FAIL + confidence
    ...
    if not api_key:
        logger.warning(
            "MVS_AUDIT_ANTHROPIC_API_KEY not set — defaulting to INSUFFICIENT_EVIDENCE for claim %s",
            claim.id,
        )
        return Verdict(
            claim_id=claim.id,
            status="INSUFFICIENT_EVIDENCE",
            confidence=0.0,
            reasoning="Anthropic API key not configured — LLM judge could not be invoked.",
            evidence=evidence,
            kg_path_present=kg_path_present,
            evidence_count=len(evidence),
        )
    ...
    logger.debug(
        "Judging claim %s (%s) with model=%s, evidence_count=%d",
        claim.id,
        claim.category,
    ...
    status, confidence, reasoning = _parse_judge_response(response.content)
